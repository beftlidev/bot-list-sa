...
if (!ClientID || isNaN(ClientID) || ClientID == client.user.id) return message.channel.send('<:carpi:855750448711467058> **Lütfen BOT ID Yazınız**').then(Message => Message.delete({timeout: 7500}))
const Prefix = args[1]
if (!Prefix) return message.channel.send('<:carpi:855750448711467058> **Lütfen Prefix Yazınız**').then(Message => Message.delete({timeout: 7500}))
const DBL = args[2]
if (!DBL) return message.channel.send('<:carpi:855750448711467058> **Lütfen DBL Durumunu Yazınız**').then(Message => Message.delete({timeout: 7500}))
if (ClientID.length < 18) return message.channel.send('<:carpi:855750448711467058> **Girdiğiniz ID Hiçbir Hesap İle Eşleşmedi (Eksik Yazmış Olabilirsiniz.).**').then(Message => Message.delete({timeout: 7500}))
if (db.fetch(`Durum_${ClientID}`) == true) return message.channel.send('<:carpi:855750448711467058> **Botunuzun Hali Hazırda Mevcut Bir Başvurusu Bulunuyor. Lütfen Bekleyin ya da Bir Yetkili İle İletişime Geçin.**').then(Message => Message.delete({timeout: 7500}))
...
if (!User.bot) return message.channel.send('<:carpi:855750448711467058> **Girdiğiniz ID Bir Bota Ait Değil.**').then(Message => Message.delete({timeout: 7500}))
const Revenge = new Discord.MessageEmbed()
.setColor('BLUE')
.setAuthor(message.author.tag,message.author.avatarURL({dynamic:true}))
.setDescription(`
<:verified:799571972727504896> \`${message.author.username}\` Adlı Kullanıcı Botunu Ekletti!
...
client.channels.cache.get(ayarlar.BOTLog).send(`<:verified:799571972727504896> **${message.author} Adlı Kullanıcı \`${User.tag}\` Adlı Botunu Sisteme Onaylanması İçin Başvurdu.**`)
